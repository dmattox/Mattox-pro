...
ScoreRect.prototype.draw = function(ctx) {
	if(game.path != null) {
		...
		if(game.pathLength > 0) {
			ctx.fillText("Path Length: " + game.pathLength + " Distance: " + game.distance ,scaleX(this.width()/2),scaleY(25));			
		}
		else
		{					
...
Game.prototype.performSearch = function(type = game.currentSearch) {
	...
	if(game.startLocation != null) {
		...
		game.messageGroupID++;   
		game.worker.postMessage({'cmd' : 'search', 'type': type, 'ID': game.messageGroupID, 'board': game.board, 'start': game.startLocation, 'goal': game.destinationLocation});
	}
